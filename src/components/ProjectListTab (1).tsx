...
export default function ProjectListTab({ projects, criterias, onVoteSuccess }: ProjectListTabProps) {
  ...
  const handleVoteSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errMsg = data?.error || (responseText.trim().length > 0 && responseText.length < 300 ? responseText.trim() : "Ocurrió un error al registrar la valoración o la URL es incorrecta.");
